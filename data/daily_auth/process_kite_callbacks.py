...
async def fetch_data(request_token: str, 
                                error: str | None,
                                API_SECRET: str,
                                REDIRECT_URI: str,
                                kc_instance: object,
                                session_file_name: str) -> HTMLResponse | JSONResponse:
    ...
    try:
        ...
        # save_session_to_file(session_data_cache)
        ...
        return HTMLResponse(
            ...
            <html>
                ...
                <body>
                    <p>Session established user: <b>{kite_user_id}</b>.</p>
                    <p>Successfully slept for 5s</p>
                    <p>You can close this page now</p>
                    <p>profile: {profile} </p>
                    <p>orders: {orders} </p>
                    <p>holdings: {holdings} </p>
                    <p>TIME AT WHICH PROCESS COMPLETED: {datetime.datetime.now(datetime.timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}</p>
                </body>
            </html>
            """
        )
